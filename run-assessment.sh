...
${GREEN}COMMANDS:${NC}
    ...
    ${CYAN}wifi${NC}             Run WiFi security assessment
    ${CYAN}all${NC}              Run all assessments (web + network + wifi)
...
${GREEN}WIFI ASSESSMENT OPTIONS:${NC}
...
${GREEN}EXAMPLES:${NC}
    ...
    # Quick WiFi security scan
    $0 wifi --scan-type quick

    # Full WiFi scan with host discovery
    $0 wifi --scan-type full --ssid "MyNetwork" --report

    # Monitor mode WiFi analysis (requires root)
    sudo $0 wifi --scan-type monitor --ssid "MyNetwork" --report

    # Run all assessments (interactive mode)
    $0 all
...
show_menu() {
    ...
    echo -e "  ${CYAN}3)${NC} WiFi Security Assessment"
    echo -e "  ${CYAN}4)${NC} All Assessments (Web + Network + WiFi)"
...
################################################################################
run_wifi_assessment() {
    echo -e "${BLUE}=== Starting WiFi Security Assessment ===${NC}"

    if [[ ! -d "$WIFI_DIR" ]]; then
        echo -e "${RED}Error: WiFi assessment directory not found: $WIFI_DIR${NC}"
        exit 1
    fi

    cd "$WIFI_DIR"

    # Check if WiFi scan script exists
    if [[ -f "wifi-scan.sh" ]]; then
        echo -e "${GREEN}Using WiFi security assessment toolkit...${NC}"
        bash wifi-scan.sh "$@"
    else
        echo -e "${YELLOW}WiFi scan script not found.${NC}"
        echo -e "${CYAN}Please run assessments manually from: $WIFI_DIR${NC}"
        echo -e "${CYAN}See WiFi-Security-Assessment-Guide.md for instructions.${NC}"
    fi
...
################################################################################
# Run Both Assessments (Web + Network - kept for backwards compatibility)
################################################################################
run_both_assessments() {
    echo -e "${BLUE}=== Running Web + Network Assessment ===${NC}"
    echo ""
...
run_all_assessments() {
    ...
    read -p "Enter web target URL (or press Enter to skip): " web_target
    read -p "Enter network target IP/range (or press Enter to skip): " network_target
    read -p "Run WiFi assessment? (y/n): " wifi_choice

    local count=0
    local total=0
    [[ -n "$web_target" ]] && ((total++))
    [[ -n "$network_target" ]] && ((total++))
    [[ "$wifi_choice" =~ ^[Yy]$ ]] && ((total++))

    if [[ -n "$web_target" ]]; then
        ((count++))
        echo ""
        echo -e "${CYAN}[$count/$total] Running Web Assessment...${NC}"
        run_web_assessment --target "$web_target"
        echo ""
    fi
    ...
    if [[ -n "$network_target" ]]; then
        ((count++))
        echo ""
        echo -e "${CYAN}[$count/$total] Running Network Assessment...${NC}"
        run_network_assessment --target "$network_target" --type quick --cve
        echo ""
    fi
    ...
    if [[ "$wifi_choice" =~ ^[Yy]$ ]]; then
        ((count++))
        echo ""
        echo -e "${CYAN}[$count/$total] Running WiFi Assessment...${NC}"
        read -p "Enter target SSID (or press Enter for quick scan): " wifi_ssid
        if [[ -n "$wifi_ssid" ]]; then
            run_wifi_assessment --scan-type full --ssid "$wifi_ssid" --report
        else
            run_wifi_assessment --scan-type quick
        fi
        echo ""
    fi
    ...
    if [[ $# -eq 0 ]]; then
        ...
        case $choice in
            ...
            2)
                ...
                ;;
            3)
                read -p "Scan type (quick/full/monitor) [quick]: " wifi_scan_type
                wifi_scan_type=${wifi_scan_type:-quick}
                read -p "Target SSID (optional): " wifi_ssid
                if [[ -n "$wifi_ssid" ]]; then
                    run_wifi_assessment --scan-type "$wifi_scan_type" --ssid "$wifi_ssid" --report
                else
                    run_wifi_assessment --scan-type "$wifi_scan_type"
                fi
                ;;
            4)
                run_all_assessments
